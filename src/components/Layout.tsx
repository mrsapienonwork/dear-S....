...
export function Layout() {
 ...
 <aside className={cn(
 "fixed md:static inset-y-0 right-0 md:left-0 z-50 flex flex-col w-72 md:w-64 border-l md:border-l-0 md:border-r border-white/5 bg-black/40 backdrop-blur-xl md:backdrop-blur-2xl p-4 lg:p-6 ease-[cubic-bezier(0.22,1,0.36,1)] shadow-2xl md:shadow-none",
 ...
 >
 <div className={cn(
 'relative z-10 flex items-center gap-4 px-4 py-3.5 md:py-3 font-medium',
 isActive ? 'text-white' : 'text-gray-400 hover:text-gray-200 group-focus-visible:ring-2 ring-white/20'
 )}>
 <item.icon className={cn(
 "w-5 h-5 ease-[cubic-bezier(0.22,1,0.36,1)] ",
 isActive ? 'text-violet-400' : 'group-'
 )} />
 ...
 {isActive ? (
 <div
 className="absolute inset-0 bg-violet-900/40 shadow-[0_0_20px_rgba(139,92,246,0.3)] border border-violet-500/50 rounded-xl z-0"
 />
 ) : (
 <div className="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 rounded-xl z-0" />
 )}
 </NavLink>
 );
 })}
 ...
 className="w-full flex items-center gap-4 px-4 py-3 rounded-xl text-gray-400 border border-transparent font-medium group hover:bg-red-500/10 hover:text-red-400 "
 >
