...
import { AppData, DailyEntry, Subject, Chapter, Habit, AppSettings, DayStatus, StudyError } from '../types';
...
interface AppContextType {
  ...
  addStudyError: (error: StudyError) => void;
  updateStudyError: (error: StudyError) => void;
  deleteStudyError: (id: string) => void;
  resetData: () => void;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    try {
      ...
      if (stored) {
        ...
        return {
          ...DEFAULT_DATA,
          ...parsed,
          studyErrors: parsed.studyErrors || [],
        };
      }
    } catch (e) {
  ...
  const importData = (importedData: AppData) => {
    // Ensure new fields exist for backward compatibility with old exports
    setData({
      ...importedData,
      studyErrors: importedData.studyErrors || [],
    });
  };

  const addStudyError = (error: StudyError) => {
    setData((prev) => ({ ...prev, studyErrors: [...(prev.studyErrors || []), error] }));
  };

  const updateStudyError = (error: StudyError) => {
    setData((prev) => ({
      ...prev,
      studyErrors: (prev.studyErrors || []).map((e) => (e.id === error.id ? error : e)),
    }));
  };

  const deleteStudyError = (id: string) => {
    setData((prev) => ({
      ...prev,
      studyErrors: (prev.studyErrors || []).filter((e) => e.id !== id),
    }));
  };
