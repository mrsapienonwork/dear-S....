...
export function CalendarView() {
  ...
  return (
    ...
    >
      ...
      <motion.div variants={staggerContainer} className="grid grid-cols-7 gap-2 md:gap-4 mb-4">
        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
          <motion.div variants={slideUpItem} key={day} className="text-center text-xs font-semibold text-gray-500 uppercase tracking-wider py-2">
            {day}
          </motion.div>
        ))}
        ...
        {monthDays.map(day => {
          ...
          return (
            <motion.div 
              ...
              whileHover={{ scale: 1.05, y: -4, transition: { duration: 0.2, ease: SYSTEM_EASE } }}
              whileTap={{ scale: 0.95 }}
              key={dateStr}
              onClick={() => navigate(`/check-in?date=${dateStr}`)}
              className={`min-h-[80px] sm:min-h-[96px] md:min-h-[128px] rounded-xl border p-1.5 sm:p-2 flex flex-col justify-between cursor-pointer transition-colors shadow-sm hover:shadow-[0_10px_20px_rgba(0,0,0,0.3)] ${bgClass} ${borderClass} ${todayBorder}`}
            >
      ...
      <motion.div variants={slideUpItem} className="flex justify-center gap-6 text-sm text-gray-400 pt-8 border-t border-white/10">
