...
import { staggerContainer, slideUpItem, SYSTEM_EASE } from '../lib/motion';
...
export function Habits() {
  ...
  return (
    ...
    >
      <motion.div variants={slideUpItem} className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
      ...
      <motion.div variants={slideUpItem} className="glass-panel border border-white/5 rounded-2xl overflow-hidden shadow-sm relative pt-2">
                ...
                {days.map(day => (
                  <th key={day.toISOString()} className="px-1.5 py-4 font-medium text-gray-500 text-center min-w-[36px] md:min-w-[40px]">
                    <div className="flex flex-col items-center">
                      ...
                      <span className={`text-xs md:text-sm mt-1.5 transition-colors ${format(day, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd') ? 'text-violet-400 font-bold bg-violet-500/10 w-6 h-6 rounded flex items-center justify-center border border-violet-500/20' : ''}`}>
                        {format(day, 'd')}
                      </span>
                    </div>
                  </th>
            ...
            >
              <AnimatePresence>
                {data.habits.map(habit => (
                  ...
                  >
                    <td className="px-5 py-4 font-medium text-gray-300 sticky left-0 glass-panel z-20 backdrop-blur-xl group-hover:bg-[#151515] transition-colors border-r border-white/5 text-xs md:text-sm shadow-[5px_0_15px_-5px_rgba(0,0,0,0.5)]">
                      {habit.name}
                    </td>
                    {days.map(day => {
                        ...
                        return (
                            <td key={dateStr} className="px-1.5 py-3 relative group/cell">
                                <div className="absolute inset-x-0 inset-y-1 bg-white/5 opacity-0 group-hover/cell:opacity-100 transition-opacity rounded" />
                                <div className="flex justify-center relative z-10">
                                    <motion.div
                                      initial={false}
                                      animate={{ 
                                        backgroundColor: isDone ? 'rgba(139, 92, 246, 0.2)' : 'transparent',
                                        borderColor: isDone ? 'rgba(139, 92, 246, 0.3)' : 'rgba(255, 255, 255, 0.05)',
                                        color: isDone ? 'rgb(167, 139, 250)' : 'transparent'
                                      }}
                                      transition={{ duration: 0.3 }}
                                      className="w-5 h-5 md:w-6 md:h-6 rounded glass-panel border flex items-center justify-center"
                                    >
                                      <AnimatePresence>
                                        {isDone && (
                                          <motion.div
                                          ...
                                          >
                                            <CheckCircle2 className="w-3.5 h-3.5 md:w-4 md:h-4 text-violet-400" />
                                          </motion.div>
                                        )}
                                      </AnimatePresence>
                                    </motion.div>
                    ...
                    <td className="px-4 py-4 font-bold text-center text-white sticky right-0 glass-panel z-20 backdrop-blur-xl group-hover:bg-[#151515] transition-colors border-l border-white/5 text-xs md:text-sm shadow-[-5px_0_15px_-5px_rgba(0,0,0,0.5)]">
      ...
      <motion.p variants={slideUpItem} className="text-[10px] md:text-xs text-gray-500 text-center mt-6 font-medium uppercase tracking-widest">Note: Edit habits on the daily Check-In page.</motion.p>
    </motion.div>
  );
}
